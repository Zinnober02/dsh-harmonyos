...
function locateDshDir() {
  ...
  for (let i = 0; i < 8 && dir !== dirname(dir); i += 1, dir = dirname(dir)) {
    ...
    if (basename(dir) === 'node_modules' && existsSync(join(dir, '@deepseek-ai', 'dsh-credentials-local', 'package.json'))) return dirname(dir);
  }
  return join(dirname(dirname(fileURLToPath(import.meta.url))), 'node_modules');
}
const DSH_DIR = locateDshDir();
if (!existsSync(join(DSH_DIR, 'node_modules', '@deepseek-ai', 'dsh-credentials-local', 'package.json')))
...
async function main() {
  ...
  die('dsh-harmonyos 发行包仅支持 patch/check; 官方核心升级请重新 npm i -g dsh-harmonyos@latest (postinstall 会自动重打补丁)');
}
